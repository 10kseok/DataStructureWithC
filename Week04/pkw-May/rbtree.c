...
node_t *rbtree_insert(rbtree *t, const key_t key)
{
  ...
  node_t *end_point = t->nil;
  new_node->key = key;

  while (current != t->nil)
  {
    // new_node가 nil을 가리킬때까지 이진탐색 반복
    end_point = current;
  ...
  new_node->parent = end_point;

  if (end_point == t->nil) // 값이 있을 줄 알았던 마지막 노드가 처음 상태 그대로 nil이라면
  {
    t->root = new_node;
  }
  else
  {
    if (key < end_point->key)
    {
      end_point->left = new_node;
    }
    else
    {
      end_point->right = new_node;
    }
  }
...
{
  node_t *uncle = NULL;
  while (new_node->parent->color == RBTREE_RED) // 내부모: Red
  {
    if (new_node->parent == new_node->parent->parent->left) // 내부모: 할배 왼쪽자식
    {
      uncle = new_node->parent->parent->right;

      if (uncle->color == RBTREE_RED) // 삼촌: Red
      {
        // 내부모와 삼촌 모두 Red니까 빨강을 할배한테 밀어올려
        new_node->parent->color = RBTREE_BLACK;
        uncle->color = RBTREE_BLACK;
        new_node->parent->parent->color = RBTREE_RED;
        new_node = new_node->parent->parent; // 이제 자리 조정은 할배 문제
        continue;
      }
      else if (new_node == new_node->parent->right) // 나: 부모의 꺾인 자식
      {
        new_node = new_node->parent;
        left_rotate(t, new_node);
      }
      // 내부모: Red, 할배 왼쪽자식 && 삼촌: Black && 나: 부모의 직선 자식
      new_node->parent->color = RBTREE_BLACK;
      new_node->parent->parent->color = RBTREE_RED;
      right_rotate(t, new_node->parent->parent); // 할배 밀어내리고 부모 올라가
    }
    else
    {
      uncle = new_node->parent->parent->left; // 내부모가 할배오른자식이니까, 삼촌은 왼자식
      ...
      else if (new_node == new_node->parent->left) // 나: 부모의 꺾인자식
      {
        new_node = new_node->parent;
        right_rotate(t, new_node);
      }
      // 내부모: Red, 할배 오른자식 && 삼촌: Black && 나: 부모의 직선 자식
      new_node->parent->color = RBTREE_BLACK;
      new_node->parent->parent->color = RBTREE_RED;
      left_rotate(t, new_node->parent->parent); // 할배 밀어내리고 부모 올라가
    }
  }
...
void rbtree_erase_fixup(rbtree *t, node_t *current)
{
  node_t *brother = NULL; // 삭제노드x의 형제노드를 가리키는 포인터brother_node를 선언.
