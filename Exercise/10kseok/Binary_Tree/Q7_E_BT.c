...
int smallestValue(BTNode *node)
{
    ...
    int smallest;
    smallest = smallestValue(node->left) > node->item ? node->item : smallestValue(node->left);
    smallest = smallest > smallestValue(node->right) ?  smallestValue(node->right) : smallest;
    return smallest;
}
