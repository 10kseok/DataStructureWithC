...
void frontBackSplitLinkedList(LinkedList *ll, LinkedList *resultFrontList, LinkedList *resultBackList)
{
	// 한 연결리스트를 두개의 연결리스트로 나눠 담는 함수
	// 연결리스트의 갯수가 홀수면, 앞쪽 연결리스트가 더 많이 담기게 한다.
	// 기준 연결리스트의 중간(또는 중간+1)까지 순회, 나머지는 뒷 연결리스트에 담아버리기.
	int total_size = ll->size;
	int mid_idx = total_size % 2 == 0 ? total_size / 2 : total_size / 2 + 1;
	int cur_idx = 0;
	...
	while (cur_node != NULL) {
		if (cur_idx++ < mid_idx) {
			// insert Front
			insertNode(resultFrontList, resultFrontList->size, cur_node->item);
		} else {
			// insert Back
			insertNode(resultBackList, resultBackList->size, cur_node->item);
		}
