...
int moveMaxToFront(ListNode **ptrHead)
{
    // 가장 큰 수를 헤드로 변경시키는 함수
	// 가장 큰 값을 가지는 노드를 찾고, 앞뒤 연결을 끊고 헤드 노드로 변경시켜준다.
	ListNode *cur_node = *ptrHead;
	ListNode *prev_node, *max_node, *next_node;
	int max_value = cur_node->item;
	max_node = cur_node;
	while (cur_node->next != NULL) {
		if (max_value < cur_node->next->item) {
			prev_node = cur_node;
			cur_node = cur_node->next;
			next_node = cur_node->next;

			max_value = cur_node->item;
			max_node = cur_node;
		} else {
			cur_node = cur_node->next;
		}
